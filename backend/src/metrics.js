...
async function computeMetrics() {
    ...
    const events = await db.all('SELECT * FROM events ORDER BY timestamp ASC');
    ...
    for (const ev of events) {
        ...
        if (wStat && wStat.lastStateTime) {
            ...
            if (wStat.lastState === 'working')
                wStat.activeTimeMs += duration;
            else if (wStat.lastState === 'idle')
                wStat.idleTimeMs += duration;
        }
        ...
        if (sStat && sStat.lastStateTime) {
            ...
            if (sStat.lastState === 'working')
                sStat.occupancyTimeMs += duration;
            else if (sStat.lastState === 'idle')
                sStat.idleTimeMs += duration;
        }
        // Update States
        if (ev.event_type === 'working' || ev.event_type === 'idle' || ev.event_type === 'absent') {
            if (wStat) {
                wStat.lastState = ev.event_type;
                wStat.lastStateTime = time;
            }
            if (sStat) {
                sStat.lastState = ev.event_type;
                sStat.lastStateTime = time;
            }
        }
        else if (ev.event_type === 'product_count') {
            if (wStat)
                wStat.totalUnits += ev.count;
            if (sStat)
                sStat.totalUnits += ev.count;
